...
///        unlink(fname);
    ...
    plot on display an copy to file later*/
    QPixmap pm;
    if (monomode == FALSE)
    pm=QPixmap::fromImage(*MainPixmap,Qt::AutoColor);
    else
    pm=QPixmap::fromImage(*MainPixmap,Qt::MonoOnly);
    //cout << "printfile=" << print_file  << " quality=" << dev.pg.dpi << " outQ=" << outputQuality << endl;
    pm.save(QString(print_file),dev.name,outputQuality);
